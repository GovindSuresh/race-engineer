...
/** Within-hue lightness ramps for stint mode: hue carries the run, shade
 *  carries the stint. Step 0 IS `runColor(slot)`, so runs mode and a
 *  first-stint series are the same colour and nothing shifts between modes.
 *
 *  Generated by stepping OKLCH lightness +0.075 per step, holding chroma at
 *  the gamut maximum where the requested chroma doesn't survive. Not by mixing
 *  hex toward white, which desaturates the four hues at visibly different
 *  rates — yellow washes out several steps before blue does.
 *
 *  FOUR steps, not five: at five, yellow and red compress against the sRGB
 *  gamut near white and the last gap falls to ΔL 0.055, under the 0.06 floor.
 *  All four ramps pass every ordinal check at four (monotone lightness, gaps
 *  >= 0.06, light end >= 4.7:1 on the panel). Past four stints in one run the
 *  ramp wraps — which is past where the chart is readable anyway.
 *
 *  **These shades REQUIRE a secondary encoding and are not valid without one.**
 *  Adjacent steps measure ΔE 7.4 under normal vision, below the 15 floor, and
 *  a lightened red lands ΔE 2.1 from base aqua under protanopia. Colour alone
 *  does not distinguish them. Every surface that uses this must also carry a
 *  non-colour channel: `stintDash` on the line charts, category labels on the
 *  boxplot axis, the series name in every legend and tooltip. */
const STINT_SHADES = [
  ["#3987e5", "#519fff", "#80b8ff", "#abd0ff"], // slot 0 — blue
  ["#199e70", "#3db686", "#59ce9d", "#74e7b5"], // slot 1 — aqua
  ["#c98500", "#e29c31", "#fbb44e", "#ffd49e"], // slot 2 — yellow
  ["#e66767", "#ff807e", "#ffaaa6", "#ffcfcc"], // slot 3 — red
] as const;

/** Colour for one comparison unit: the run's hue, the stint's shade.
 *  `stintIndex` 0 returns exactly `runColor(slot)`. */
export function stintColor(slot: number, stintIndex: number): string {
  const ramp = STINT_SHADES[slot % STINT_SHADES.length];
  return ramp[stintIndex % ramp.length];
}

/** ECharts `lineStyle.type` for one stint index — the secondary encoding the
 *  shade ramp depends on. Carries the same distinction as the shade through a
 *  channel colour vision can't take away, which is what lets two steps only
 *  ΔE 7.4 apart still read as different series. */
export function stintDash(stintIndex: number): number[] | "solid" {
  const patterns = ["solid", [7, 4], [2, 3], [11, 4, 2, 4]] as const;
  const pattern = patterns[stintIndex % patterns.length];
  return pattern === "solid" ? "solid" : [...pattern];
}
